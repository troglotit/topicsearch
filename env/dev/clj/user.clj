(ns user
  ...
  (:require
    ...
    [troglotit.topicsearch.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (troglotit.topicsearch.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (troglotit.topicsearch.config/system-config {:profile :test})
                                  (ig/prep)))))
